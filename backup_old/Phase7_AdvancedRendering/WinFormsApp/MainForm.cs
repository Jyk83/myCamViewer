...
namespace CamViewerPOC
{
    ...
    public partial class MainForm : Form
    {
        ...
        private Button btnRenderSettings;
        ...
        private void BtnRenderSettings_Click(object sender, EventArgs e)
        {
            using (RenderSettingsForm settingsForm = new RenderSettingsForm())
            {
        ...
        /// <summary>
        /// Phase 7: Open Real-time Trace Test dialog
        /// </summary>
        private void BtnTraceTest_Click(object sender, EventArgs e)
        {
            ...
            var progressManager = viewerControl.GetCuttingProgressManager();
            if (progressManager == null)
            {
                MessageBox.Show("CuttingProgressManager가 초기화되지 않았습니다.", "트레이스 테스트",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            using (TraceTestForm traceForm = new TraceTestForm(progressManager, program, viewerControl))
            {
        ...
        private void SetupSimulationPanel()
        {
            ...
            // Render settings button (Phase4)
            btnRenderSettings = new Button
            {
                Text = "Render Settings (렌더링 설정)",
                Location = new Point(10, 850),
                Size = new Size(330, 40),
                BackColor = Color.FromArgb(102, 51, 153), // Purple
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Cursor = Cursors.Hand,
                Font = new Font("Segoe UI", 10, FontStyle.Bold)
            };
            btnRenderSettings.Click += BtnRenderSettings_Click;
