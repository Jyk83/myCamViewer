...
namespace RealtimeITagControl
{
    ...
    public partial class CamViewerControl : UserControl
    {
        ...
        private void DrawContour(Contour contour, float offsetX, float offsetY, int partIndex, int contourIndex)
        {
            ...
            if (contour.AllSegments != null)
            {
                ...
                foreach (PathSegment segment in contour.AllSegments)
                {
                    ...
                    double elementProgress = 0.0;  // Phase 13: 진행 중인 엘리먼트의 진행률 (0~1)
                    if (progressManager != null && progressManager.HasCuttingProgress)
                    {
                        ...
                        // Phase 13: 진행 중인 엘리먼트의 진행률 가져오기
                        if (isInProgress)
                        {
                            elementProgress = progressManager.ElementProgress;  // 0~1 범위
                        }
                    }
                    ...
                    else
                    {
                        ...
                        // Phase 13: isInProgress도 기본 색상 사용 (덮어쓰기 방식)
                    ...
                    // Phase 13: 덮어쓰기 방식 렌더링
                    // 1) 먼저 전체 엘리먼트를 기본 색상으로 그림
                    DrawPathSegment(segment, offsetX, offsetY, finalColor[0], finalColor[1], finalColor[2], segmentWidth);
                    
                    // 2) 진행 중인 경우, 진행된 부분만 빨간색으로 덮어그림
                    if (isInProgress && elementProgress > 0.0 && elementProgress < 1.0)
                    {
                        float[] progressColor = settings.GetColorAsFloat(settings.CuttingInProgressColor);
                        DrawPartialPathSegment(segment, offsetX, offsetY, 
                            progressColor[0], progressColor[1], progressColor[2], segmentWidth, elementProgress);
                    }
        ...
        /// <summary>
        /// Phase 13: Draw partial path segment (for in-progress element)
        /// </summary>
        private void DrawPartialPathSegment(PathSegment segment, float offsetX, float offsetY,
                                           float r, float g, float b, float lineWidth, double progress)
        {
            if (segment == null || progress <= 0.0) return;
            ...
            if (segment is LineSegment)
            {
                ...
                // 진행 중인 부분만 렌더링
                float endX = x1 + (x2 - x1) * (float)progress;
                float endY = y1 + (y2 - y1) * (float)progress;
                
                NativeRenderer.DrawLine(x1, y1, endX, endY, r, g, b, lineWidth);
            }
            else if (segment is ArcSegment)
            {
                ArcSegment arc = (ArcSegment)segment;
                ...
                float partialEndAngleRad = startAngleRad + totalAngle * (float)progress;
                
                NativeRenderer.DrawArc(centerX, centerY, radius,
                                      startAngleRad, partialEndAngleRad,
                                      arc.Clockwise ? 1 : 0,
                                      r, g, b, lineWidth);
            }
        }
        ...
        private void SetupSimulationEngine()
        {
            ...
            // Phase 13: Timer는 StartSimulation()에서만 시작
            // redrawTimer.Start(); // 제거
        ...
        public void StartSimulation()
        {
            ...
            try
            {
                ...
                // Phase 13: 시뮬레이션 시작 시 redraw 타이머 시작
                redrawTimer.Start();
            }
            catch (Exception ex)
            {
        ...
        public void PauseSimulation()
        {
            if (simulationEngine.State == SimulationState.Running)
            {
                ...
                // Phase 13: 시뮬레이션 일시정지 시 redraw 타이머 정지
                redrawTimer.Stop();
            }
        }
        ...
        public void ResumeSimulation()
        {
            if (simulationEngine.State == SimulationState.Paused)
            {
                ...
                // Phase 13: 시뮬레이션 재개 시 redraw 타이머 재시작
                redrawTimer.Start();
            }
        }
        ...
        public void StopSimulation()
        {
            ...
            // Phase 13: 시뮬레이션 정지 시 redraw 타이머 정지
            redrawTimer.Stop();
        ...
        private void InvalidateDirtyRegion(Rectangle dirtyRect)
        {
            if (OpenGLSettings.CurrentMode == OpenGLRenderMode.DirtyRegion)
            {
                ...
                {
                    // Phase 14.2: Record dirty region invalidate
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                try
                {
                    ...
                    // 4. Phase 8.1: Cleanup text renderer
                    if (textRendererInitialized)
                    {
                        try
                        {
                    ...
                    // 6. RenderPanel 및 자식 컨트롤 정리 (WinCC 관리로 위임)
