...
// Phase 13: View Direction (원점 위치)
// 1 = RightBottom (우하단 원점), 2 = LeftBottom (좌하단 원점, 기본)
static int g_viewDirection = 2;
...
RENDERER_API void RenderFrame() {
	...
	for (const auto& shape : g_shapes) {
		...
		if (shape.type == Shape::RECTANGLE) {
			// Phase 13: Swap coordinates for ViewDirection Type 1
			...
			// Phase 13: Swap coordinates for ViewDirection Type 1
			float x = shape.x, y = shape.y;
			if (g_viewDirection == 1) {
				std::swap(x, y);
			}

			DrawFilledCircle(x, y, shape.radius);
			...
			for (int i = 0; i < 32; i++) {
				...
				float cx = shape.radius * cosf(theta);
				float cy = shape.radius * sinf(theta);
...
// Phase 13: Set view direction (origin position)
RENDERER_API void SetViewDirection(int direction) {
	if (direction == 1 || direction == 2) {
		g_viewDirection = direction;
	}
}
...
RENDERER_API void BeginMPFRenderWithBackground(float bgR, float bgG, float bgB) {
	...
	// Phase 13: Adjust glOrtho based on ViewDirection
	float left, right, bottom, top;

	if (g_viewDirection == 1) {
		// Type 1: RightBottom origin (우하단 원점)
		// Phase 13: Coordinates are swapped in Draw functions
		// glOrtho uses same mapping as Type 2, but coordinates will be MPF-swapped
		//
		// MPF X (위↑) will be passed as OpenGL Y (after swap in Draw functions)
		// MPF Y (좌←) will be passed as OpenGL X (after swap in Draw functions)
		//
		// Also need to flip axes for RightBottom origin:
		//   X axis: left(+) to right(-)
		//   Y axis: bottom(-) to top(+) - same as Type 2

		left = viewWidth / 2 + g_panX;     // Flip X: left > right
		right = -viewWidth / 2 + g_panX;
		bottom = -viewHeight / 2 + g_panY;  // Normal Y
		top = viewHeight / 2 + g_panY;

		glOrtho(left, right, bottom, top, -1.0, 1.0);
	}
	else {
		// Type 2: LeftBottom origin (좌하단 원점, 기본 OpenGL)
		// MPF X → OpenGL X
		// MPF Y → OpenGL Y
...
RENDERER_API void DrawArc(float centerX, float centerY, float radius, float startAngle, float endAngle,
	...
	// Phase 13: Transform for ViewDirection Type 1
	if (g_viewDirection == 1) {
		// Type 1 coordinate transformation:
		//   Type1_X = Type2_Y
		//   Type1_Y = -Type2_X
		//
		// This means:
		//   cos(angle), sin(angle) -> sin(angle), -cos(angle)
		//   Which equals: cos(90° - angle), sin(90° - angle)
		//
		// Therefore: angle_new = 90° - angle_old
		//
		// Also, the Y-axis flip reverses the rotation direction

		std::swap(centerX, centerY);

		// Transform angles: angle -> 90° - angle
		float newStartAngle = PI / 2.0f - startAngle;
		float newEndAngle = PI / 2.0f - endAngle;
		startAngle = newStartAngle;
		endAngle = newEndAngle;

		// Reverse clockwise direction due to Y-axis flip
		clockwise = !clockwise;
	}
...
RENDERER_API void DrawPartNumber(double posX, double posY, unsigned int number,
	...
	// Phase 13: Flip text horizontally for Type 1 (to counter X-axis flip in glOrtho)
	if (g_viewDirection == 1) {
		glPushMatrix();
		glTranslated(posX, posY, 0.0);
		glScaled(-1.0, 1.0, 1.0);  // Flip X axis
		glTranslated(-posX, -posY, 0.0);
	}
	TextRenderer_DrawNumber(posX, posY, number, scale, r, g, b);


	// Phase 13: Restore matrix if flipped
	if (g_viewDirection == 1) {
		glPopMatrix();
	}
	// Restore state
